...
public class DetailActivity extends AppCompatActivity {
    ...
    private void populateUI(Sandwich sandwich) {
        if (sandwich.getAlsoKnownAs().size() == 0) {
            ...
            mAlsoKnownAsTv.setText(android.text.TextUtils.join(", ", sandwich.getAlsoKnownAs()));
        }

        if (sandwich.getIngredients().size() == 0) {
            mIngredientsTv.setText(R.string.no_value);
        } else {
            mIngredientsTv.setText(android.text.TextUtils.join(", ", sandwich.getIngredients()));
        }
        ...
        if (sandwich.getDescription().length() == 0) {
            mDescriptionTv.setText(R.string.no_value);
        } else {
